...
from views.modificarUsuarioEmpleado import ModificarUsuarioE
...
class MenuUser(QMainWindow):   
    def __init__(self, parent=None):
        super(MenuUser, self).__init__(parent)
        ...
        # self.btn_regenerar_Todo.clicked.connect(self.mostrar_historial_usuario_actual)
        ...
        self.mostrar_historial_usuario_actual()



        # Botones
        self.btn_actualizarH.clicked.connect(self.boton_actualizar_historial)

        # self.comboBox_historial_usuario.currentIndexChanged.connect(self.filtrar_por_usuario)
        self.fecha_historial.dateChanged.connect(self.activar_filtro_por_fecha)
...
#Venta de Entradas
    ...
    def mostrar_pelicula(self):
        ...
        if self.funciones_detalladas and 0 <= self.indice_pelicula < len(self.funciones_detalladas):
            ...
            #Formatear el precio con separador de miles y punto decimal
            precio_formateado = "${:,.2f}".format(float(funcion_actual['precio'])).replace(",", ".")
            ...
            self.Butacas_disponibles.setReadOnly(True)
        else:
    ...
    def abrir_modificar_usuario(self):
        self.modificar_usuario = ModificarUsuarioE()
    ...
    def cargar_usuarios_en_tabla(self):
        """
        Carga solo la información del usuario en sesión.
        """
        try:
            ...
            if not usuario:
                QMessageBox.warning(self, 'Advertencia', 'No se encontró información para el usuario logueado.')
                return
            ...
            self.tableWidget_usuarios.insertRow(0)

            nombre_usuario = usuario[1]       # NombreUsuario
            grupo = usuario[3]                # Grupo
    ...
    def mostrar_historial_usuario_actual(self):
        """Muestra el historial del usuario en sesión, con o sin filtro."""
        try:
            session = UserSession()
            user_id = session.get_user_id()

            if user_id is None:
                QMessageBox.warning(self, 'Advertencia', 'No hay un usuario en sesión.')
                return

            # Obtener todo el historial del usuario
            historial = self.db.obtener_historial_usuario(user_id)

            # Aplicar filtro si está activado
            if self.filtro_fecha_activado:
                print("Aplicando filtro por fecha...")
                ...
                print("Cargando historial sin filtro...")
            ...
            for row_number, row_data in enumerate(historial_ordenado):
                ...
                self.tableWidget_historial.setItem(row_number, 0, QTableWidgetItem(str(fecha_hora)))
                self.tableWidget_historial.setItem(row_number, 1, QTableWidgetItem(accion))

            self.tableWidget_historial.resizeColumnsToContents()
            self.tableWidget_historial.resizeRowsToContents()

            # Desactivar el filtro después de cargar todo el historial sin filtro
            if not self.filtro_fecha_activado:
                self.filtro_fecha_activado = False

        except Exception as e:
            log(e, "error")
            QMessageBox.critical(self, 'Error', 'No se pudo cargar el historial.')


    def activar_filtro_por_fecha(self):
        """Activa el filtro por fecha y recarga los datos del historial."""
        print("Filtro por fecha activado")
        self.filtro_fecha_activado = True
        self.mostrar_historial_usuario_actual()

    def desactivar_filtro_por_fecha(self):
        """Desactiva el filtro por fecha y recarga todo el historial."""
        print("Filtro por fecha desactivado")
        self.filtro_fecha_activado = False
        self.mostrar_historial_usuario_actual()
    
    def boton_actualizar_historial(self):
        self.filtro_fecha_activado = False
        self.mostrar_historial_usuario_actual()
    ...
    def cargar_Funciones_en_tabla(self):
        ...
        try:
            ...
            for row_number, row_data in enumerate(funciones):
                ...
                # Formatear el precio (columna 4 → row_data[4])
                precio_formateado = "{:,.2f}".format(float(row_data[4])).replace(",", ".")

                datos_visibles = [row_data[0], pelicula, row_data[2], row_data[3], precio_formateado, butacas_vendidas]
